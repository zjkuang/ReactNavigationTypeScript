...
export const DemoUseStateView = (props: DemoUseStateViewProp) => {
  ...
  return (
    <View style={styles.baseView}>
      ...
      {(state === 'Welcome') && <TouchableOpacity
        onPress={() => {
      ...
      {(state === 'Order Placed') && <TouchableOpacity
        onPress={() => {
            setState('Food Consumed');
        }}>
        <Text style={{color: color.iOSButtonColorLightTheme}}>Eat The Meal</Text>
      </TouchableOpacity>}

      {(state === 'Food Consumed') && <TouchableOpacity
        onPress={() => {
            setState('Bill Paid');
        }}>
        <Text style={{color: color.iOSButtonColorLightTheme}}>Pay The Bill</Text>
      </TouchableOpacity>}
    </View>
  );
};
